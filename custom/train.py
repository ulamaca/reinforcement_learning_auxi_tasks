...
if __name__ == '__main__':
    ...
    with U.make_session(4) as sess:
        ...
        # (TODO) remove it, for debugging, referring to the original graph structure
        logs_path = os.path.join(os.getcwd(), 'tb_dqn')
        ...
        while True:
            ...
            if (num_iters > max(5 * args.batch_size, args.replay_buffer_size // 20) and
                ...
                error = np.mean(td_errors * weights)
                errors_summary_temp.append(error)
            ...
            if num_iters % args.target_update_freq == 0:
                ...
                if num_iters > max(5 * args.batch_size, args.replay_buffer_size // 20):
                    ...
                    errors_summary.append(np.mean(errors_summary_temp))
                    std_errors_summary.append(np.std(errors_summary_temp, axis=0))
                    errors_summary_temp = []
                    rewards_100pi_summary_temp = []
                    if num_iters > N_temp_saving and num_iters // N_temp_saving:
                        with open(os.path.join(savedir, 'error_summary_temp' + str(num_iters) + '.pickle'), 'wb') as f:
                            pickle.dump([errors_summary, std_errors_summary, rewards_100pi_summary], f)
